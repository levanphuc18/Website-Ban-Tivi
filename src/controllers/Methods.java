...
//		String hql = "FROM ProductEntity p WHERE p.description LIKE :keyword OR p.category.name LIKE:keyword ORDER BY p.dateAdded DESC";
...
//	}
	...
	public List<ProductEntity> searchForProduct(String keyword) {
		Session session = factory.getCurrentSession();
	    String[] keywords = keyword.split("\\s+"); // Tách từ khóa thành các từ riêng lẻ
	    
	    StringBuilder hqlBuilder = new StringBuilder("FROM ProductEntity p WHERE ");
	    
	    for (int i = 0; i < keywords.length; i++) {
	        if (i > 0) {
	            hqlBuilder.append(" AND ");
	        }
	        hqlBuilder.append("p.name LIKE :keyword").append(i);
	    }

		...
		<div class="main__new-product section px-32 my-6">
			<div class="section__heading mb-8">
				<div class="product-navigation font-bold mb-5">
					<h3 class="text-indigo-500 font-bold text-xl">Kết quả tìm
						kiếm cho: ${keyword}</h3>

				</div>
				<hr class="border-none bg-indigo-500" style="height: 1px;">
				<c:if test="${pagedListHolder.pageList.size()==0}">
					<p class="mt-5">Có vẻ không có sản phẩm phù hợp với từ
						khóa.</p>
					<p class="mb-2">
						Nhấn vào <a href="${applicationScope.storeIndexPage}"
							class="inline-block font-bold text-indigo-500 hover:text-indigo-300 transition-colors">đây</a>
						để quay lại trang chủ.
					</p>
				</c:if>
			</div>
			...
			<div class="new-product__list flex flex-wrap">
				...
				<c:url value="store/search/${keyword}/" var="pagedLink">
					...
					<div class="item w-1/4 px-3 mb-4 flex flex-col justify-between">
						<div>
							<div class="relative item-menu-container overflow-hidden">
								<a href="${applicationScope.productDetailPage}/${pd.id}"
									...
									src="${pd.images[0].image}" alt="${pd.id}"
								...
								<div
									...
									<a
										href="store/insert-to-favlist/${pd.id}/searchseperator${keyword}seperatorquestionmarkp=${pagedListHolder.page}">
									...
									<a
										href="store/shopping-cart/insert-into-cart/${pd.id}/searchseperator${keyword}seperatorquestionmarkp=${pagedListHolder.page}"><i
						...
						<div>
							<span class="item__price font-bold"><fmt:setLocale
									...
									value="${pd.price}" type="currency" /></span>

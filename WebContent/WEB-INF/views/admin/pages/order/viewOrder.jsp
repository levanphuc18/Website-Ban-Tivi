	...
	<main class="content">
				...
				<ol class="breadcrumb breadcrumb-dark breadcrumb-transparent">
                  ...
                  <path stroke-linecap="round" stroke-linejoin="round"
					...
					<li class="breadcrumb-item active" aria-current="page"><a
						href="${requestScope['javax.servlet.forward.request_uri']}">${order.id }</a></li>
				</ol>
			</nav>
		</div>
					...
					<div class="card-body">
						...
						<div class="row">
							<div class="col-12">
								...
								<ul>
									<li><small>Ngày đặt:</small> <span class="fw-bold">
											<fmt:formatDate value="${order.orderDate}"
												pattern="dd/MM/yyyy" />
									</span></li>
									<li><small>Giờ đặt:</small> <span class="fw-bold">
											<fmt:formatDate value="${order.orderDate}" pattern="HH:mm:ss" />
									</span></li>
									<li><small>Tổng số sản phẩm:</small> <span
										class="fw-bold text-secondary">${order.getTotalQuantity() }</span></li>
									<li><small>Tổng thành tiền:</small> <span
										class="fw-bold text-success"> <fmt:setLocale
								...
								<ul>
									...
									<li>
										<p class="text-truncate my-0">
											<small>Địa chỉ:</small> <span class="ms-1 fw-bold text-info">
												${order.shipAddress } </span>
										</p>
									</li>
									<li><small>Số điện thoại:</small> <span
										class="fw-bold text-danger">${order.shipPhone }</span></li>
									<li><small>Ghi chú:</small> <span
										class="fw-bold text-gray-400">${order.shipNote }</span></li>
								</ul>
							</div>
						</div>
					</div>
		...
		<div
			...
			<table class="table table-hover table-responsive" id="datatable">
				...
				<tbody>
					<!-- Item -->
					<c:forEach items="${pagedListHolder.pageList}" var="invoiceDetail"
						varStatus="i">
						<tr>
							<td><span class="fw-normal text-wrap">${i.count}</span></td>
							<td><a href="${applicationScope.productPage}/${product.id}"
								class="fw-bold"> ${invoiceDetail.getProduct().id} </a></td>
							...
							<td><span class="fw-bold text-wrap"> <fmt:setLocale
										value="vi_VN" scope="session" /> <fmt:formatNumber
										value="${invoiceDetail.getProduct().price}" type="currency" />
							</span></td>
							<td><span class="fw-bold text-wrap"> <fmt:setLocale
										value="vi_VN" scope="session" /> <fmt:formatNumber
										value="${invoiceDetail.getProduct().price * invoiceDetail.quantity}"
										type="currency" />
							</span></td>
							<%--
		...
		<footer class="bg-white rounded shadow py-3 px-4 mb-4 mt-4">
			<div class="row d-flex align-items-center">
				...
				<div class="col-12 col-md-10 col-xl-10 text-center text-lg-start">
					<!-- List -->
					<form action="${applicationScope.ordersPage }/${order.id}"
						method="post">
						<ul
							class="list-inline list-group-flush list-group-borderless text-md-end mb-0">
							<li class="list-inline-item px-0 px-sm-2">
								<button class="btn ${order.state == -1 ? 'btn-danger' : '' }"
									name="state" value="-1">Hủy đơn hàng</button>
							</li>

							<li class="list-inline-item px-0 px-sm-2">
								<button class="btn ${order.state == 0 ? 'btn-info' : '' }"
									name="state" value="0">Đơn hàng mới đến</button>
							</li>

							<li class="list-inline-item px-0 px-sm-2">
								<button class="btn ${order.state == 1 ? 'btn-tertiary' : '' }"
									name="state" value="1">Khách hàng đã xác nhận</button>
							</li>
							<li class="list-inline-item px-0 px-sm-2">
								<button class="btn ${order.state == 2 ? 'btn-secondary' : '' }"
									name="state" value="2">Đang xử lý</button>
							</li>
							<li class="list-inline-item px-0 px-sm-2">
								<button class="btn ${order.state == 3 ? 'btn-success' : '' }"
									name="state" value="3">Đã hoàn thành</button>
							</li>

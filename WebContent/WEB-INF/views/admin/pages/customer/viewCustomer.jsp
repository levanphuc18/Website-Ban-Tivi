			...
			<nav aria-label="breadcrumb" class="d-none d-md-inline-block">
                ...
                <path stroke-linecap="round" stroke-linejoin="round"
					...
					<li class="breadcrumb-item active" aria-current="page"><a
						href="${applicationScope.customerPage }/${customer.id }">${customer.id }</a></li>
				</ol>
			</nav>
						...
						<div class="row">
							<div class="col-12">
								<ul class="">
									...
									<li><small>Họ tên:</small> <span class="fw-bold">${customer.lastname }
											${customer.firstname }</span></li>
									<li>
										<p class="text-truncate my-0">
											<small>Địa chỉ:</small> <span
												class="ms-1 fw-bold text-primary">
												${customer.address} </span>
										</p>
									</li>
									<li><small>Số điện thoại:</small> <span
										class="fw-bold text-primary">${customer.phone }</span></li>
									<li><small>Email:</small> <span
										class="fw-bold text-primary">${customer.email }</span></li>
									<li><small>Ngày đăng ký:</small> <span
										class="fw-bold text-primary">${customer.dateAdded }</span></li>
								</ul>
							</div>
						</div>
					</div>
								...
								<ul class="">
									<li class="">
										<div class="d-flex align-items-center">
											<small>Sản phẩm yêu thích:</small> <span
												class="fw-normal text-danger d-flex align-items-center">
												<svg class="icon icon-xxs me-1 ms-2" fill="none"
									...
									<li><small>Tổng số đơn hàng:</small> <span
										class="fw-bold text-bold">${customer.orders.size()}</span></li>

									<li><small>Lần đặt hàng cuối:</small> <span
										class="fw-bold"> <fmt:formatDate
												value="${customer.orders.get(0).orderDate}"
												pattern="dd/MM/yyyy HH:mm:ss" />
									</span></li>
									<li><small>Tổng số tiền đặt hàng:</small> <span
										class="fw-bold text-success"> <fmt:setLocale
												value="vi_VN" scope="session" /> <fmt:formatNumber
												value="${customer.getTotalAmountOfOrders() }"
												type="currency" />
									</span></li>
								</ul>
							</div>
			...
			<table class="table table-hover table-responsive" id="datatable">
				...
				<tbody>
					...
					<c:forEach items="${customer.orders}" var="order">
						<tr>
							<td><a href="${applicationScope.ordersPage}/${order.id}"
								class="fw-bold"> ${order.id} </a></td>
							<td><span class="fw-normal text-wrap"> <fmt:formatDate
										value="${order.orderDate}" pattern="dd/MM/yyyy" />

							</span></td>
							<td><span class="fw-bold text-wrap">
									${order.shipAddress} </span></td>
								...
								<div class="btn-group">
									...
									<div class="dropdown-menu py-0">
										<a class="dropdown-item rounded-top"
											href="${applicationScope.ordersPage}/${order.id}"> <i

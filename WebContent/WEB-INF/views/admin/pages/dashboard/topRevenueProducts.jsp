	...
	<main class="content">
		...
		<div
			...
			<div class="d-block mb-4 mb-md-0">
				<a href="${applicationScope.dashboardPage }"
					class="btn btn-gray-200"> <svg class="icon icon-xs me-2"
		...
		<div class="card border-0 shadow">
			<div
				...
				<div
					class=" h2 fs-5 fw-bold mb-0 text-gray-800 d-flex align-items-center">
				...
				<div class="table-responsive">
					<table class="table align-items-center table-flush"
						...
						<thead class="thead-light">
							<tr>
								<th class="border-bottom rounded-start" scope="col">Xếp
									hạng</th>
						...
						<tbody>
							<c:if test="${pagedListHolder.pageList.isEmpty()}">
								<tr>
									<td class="fw-bolder text-gray-500" rowspan="0">Chưa có
										sản phẩm nào</td>
								</tr>
							</c:if>
							...
							<c:if test="${!pagedListHolder.pageList.isEmpty()}">
								...
								<c:forEach items="${pagedListHolder.pageList}" var="product"
									varStatus="i">
									<tr>
										<td class="text-gray-900"><c:set var="index"
												value="${(pagedListHolder.page) * pagedListHolder.pageSize + i.count}"></c:set>
											<h3
												class="my-0 ${index == 1 ? 'text-danger' : index == 2 ? 'text-secondary' : index == 3 ? 'text-tertiary' : 'text-gray-400' }">
												${index}</h3></td>
										<th class="text-gray-900 text-truncate" scope="row"
											colspan="3"><a
											href="${applicationScope.productPage }/${product.getProduct().id}">
												${product.getProduct().name } </a></th>
										<td class="fw-bolder text-gray-500"><fmt:setLocale
												value="vi_VN" scope="session" /> <%--                         <fmt:formatNumber value="${product[0].getGrossRevenueInOrdersOfThisMonth(product[0].id) }" type="currency" /> --%>
											<fmt:formatNumber value="${product.thisMonthRevenue }"
												type="currency" /></td>
										<td class="fw-bolder text-gray-500"><span
											class="d-flex align-items-center"> <c:set
													var="bounceRate" value="${product.growthRatePercentage}"></c:set>
												<c:if test="${bounceRate > 0}">
													<svg class="icon icon-xs text-success me-2" fill="none"
														stroke="currentColor" viewBox="0 0 24 24"
														xmlns="http://www.w3.org/2000/svg">
														<path stroke-linecap="round" stroke-linejoin="round"
															stroke-width="2" d="M8 7l4-4m0 0l4 4m-4-4v18"></path></svg>
													<fmt:setLocale value="vi_VN" scope="session" />
													<fmt:formatNumber value="${bounceRate}" type="number" />%
                    	</c:if> <c:if test="${bounceRate < 0}">
													<svg class="icon icon-xs text-danger me-2" fill="none"
														stroke="currentColor" viewBox="0 0 24 24"
														xmlns="http://www.w3.org/2000/svg">
														<path stroke-linecap="round" stroke-linejoin="round"
															stroke-width="2" d="M16 17l-4 4m0 0l-4-4m4 4V3"></path></svg>
													<fmt:setLocale value="vi_VN" scope="session" />
													<fmt:formatNumber value="${-bounceRate}" type="number" />%
                    	</c:if> <c:if test="${bounceRate == 0}">
													<svg class="icon icon-xs text-gray-400 me-2" fill="none"
														stroke="currentColor" viewBox="0 0 24 24"
														xmlns="http://www.w3.org/2000/svg">
														<path stroke-linecap="round" stroke-linejoin="round"
										...
										<td class="fw-bolder text-gray-500"><fmt:setLocale
												...
												value="${product.allTimeRevenue}" type="currency" /></td>

...
<body>
	...
	<main class="content">
			...
			<nav aria-label="breadcrumb" class="d-none d-md-inline-block">
				<ol class="breadcrumb breadcrumb-dark breadcrumb-transparent">
					...
					<li class="breadcrumb-item active" aria-current="page"><a
						href="${requestScope['javax.servlet.forward.request_uri']}">${category.id }</a></li>
				</ol>
			</nav>
				...
				<div class="card border-0 shadow">
					<div class="card-body">
						<div class="row d-flex">
							...
							<div
								class="d-flex col-md-9 flex-md-row col-sm-8 flex-column mt-2">
								<div
									class="col-md-10 col-12 d-flex flex-column justify-content-center">
									<div class="">
										<h3 class="fw-extrabold mb-0">${category.name }</h3>
										<p class="text-gray-400 mb-2">${!category.description ? "Không có mô tả cho danh mục" : category.description}
										</p>
									</div>

									<div class="d-flex mt-1">
										<div>
											<span class="small text-gray-500">Ngày thêm:</span> <span
												class="fw-normal"> <fmt:setLocale value="vi_VN"
													scope="session" /> <fmt:formatDate
													value="${category.dateAdded}" pattern="dd/MM/yyyy" />
											</span>
										</div>
									</div>
									...
									<div class="d-flex mt-1">
										<div>
											<span class="small text-gray-500">Số lượng sản phẩm
												thuộc danh mục:</span> <span class="text-success fw-bolder fs-6">${category.products.size() }</span>
										</div>
									</div>
								</div>

								<div class="col-md-2 col-12 mt-2 d-flex flex-column">
									<a href="${applicationScope.categoryPage }/edit/${category.id}"
										class="btn btn-block btn-gray-800 mb-3 d-inline-flex align-items-center justify-content-center">
									...
									<!-- Button Delete -->
									<button
										class="btn btn-block btn-danger mb-3 d-inline-flex align-items-center justify-content-center"
		...
		<div
			class="card card-body border-0 shadow table-wrapper table-responsive overflow-hidden">
			<table class="table table-hover table-responsive" id="datatable">
				...
				<tbody>
					...
					<c:forEach items="${pagedListHolder.pageList}" var="product">
						<tr>
							<td><a href="${applicationScope.productPage}/${product.id}"
								class="fw-bold"> ${product.id} </a></td>
							...
							<td><span class="fw-bold text-wrap"> <fmt:setLocale
							...
							<td><span class="fw-normal text-wrap"> <img
									src="${!product.images.isEmpty() ? product.images.get(0).image : ''}"
							...
							<td class="text-center">
								<div class="btn-group">
									<button
										...
										aria-expanded="false">
										<svg class="icon icon-xxs" stroke="currentColor"
											viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                         <path stroke-linecap="round"
												stroke-linejoin="round" stroke-width="2"
									...
									<div class="dropdown-menu py-0">
										<a class="dropdown-item rounded-top"
											href="${applicationScope.productPage}/${product.id}"> <i
											class="bi bi-eye-fill dropdown-icon text-gray-400 me-2 icon-sm"></i>
											Chi tiết
										</a> <a class="dropdown-item text-warning"
											href="${applicationScope.productPage}/edit/${product.id}">
											<i class="bi bi-pencil-square dropdown-icon me-2"></i> Chỉnh
											sửa
										</a>
	...
	<%@include
		file="/WEB-INF/views/admin/pages/category/deleteCategoryModal.jsp"%>

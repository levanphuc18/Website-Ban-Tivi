...
<body>
	...
	<main class="content">
				...
				<ol class="breadcrumb breadcrumb-dark breadcrumb-transparent">
                  ...
                  <path stroke-linecap="round" stroke-linejoin="round"
					...
					<li class="breadcrumb-item active" aria-current="page"><a
						href="${applicationScope.addInvoicePage }">Tạo hoá đơn</a></li>
				</ol>
			</nav>
					...
					<form action="${requestScope['javax.servlet.forward.request_uri']}"
						...
						<div class="input-group me-2 me-lg-3 fmxw-300">
							<input type="text" class="form-control" placeholder="Tìm kiếm"
								name="search" /> <a class="input-group-text"
				...
				<div
                    ...
                    <c:forEach items="${invoice.invoiceDsetails}" var="invoiceDetail" varStatus="j">
                      ...
                      <input class="form-control d-none" name="rows" value="${i.count}" /> --%>
								<tr
									class="${invoice.hasProduct(product.id) ? 'table-active' : '' }">
									<td><span class="fw-bold text-wrap"> <a
											href="${applicationScope.productPage}/${product.id}">
												${product.id} </a>
									</span></td>
									...
									<td><span class="fw-normal"> <img class="img-fluid"
                        ...
                        --%>
									...
									<td><span class="fw-bold text-wrap"> <fmt:setLocale
												...
												value="${invoice.getPrice(product.id)*invoice.getQuantity(product.id) == 0 ? 0 : invoice.getPrice(product.id)*invoice.getQuantity(product.id)}"
												type="currency" />
									</span></td>
									...
									<td><span class="fw-normal text-wrap">
											<button class="dropdown-item rounded-top" id="btn-add"
												...
												<i
													class="bi bi-plus-circle dropdown-icon text-gray-400 me-1 icon-sm"></i>
												Cập nhật
											</button>
									</span> <span class="fw-normal text-wrap"> <a
											class="dropdown-item rounded-top"
											href="${applicationScope.invoicePage }/add/update"> <i
												class="bi bi-plus-circle dropdown-icon text-gray-400 me-1 icon-sm"></i>
												Xoá
										</a>
									</span></td>
		...
		<%@include
			file="/WEB-INF/views/admin/pages/invoice/pageNavigation.jsp"%>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<form action="${requestScope['javax.servlet.forward.request_uri']}"
						...
						<div class="row mb-4">
							<div class="col-6">
								<label for="productName">Mã sản phẩm</label> <input type="text"
							...
							<div class="col-6">
								<label for="productName">Tên sản phẩm</label> <input type="text"
									class="form-control" id="productName" readonly="readonly" />
							</div>
						</div>

						<div class="row mb-4">
							<div class="col-6">
								<label for="quantity">Danh mục</label> <input value=""
									type="text" class="form-control" id="category"
									readonly="readonly" />
							</div>
							<div class="col-6">
								<label for="quantity">Số lượng tồn</label> <input type="text"
									class="form-control" name="oldQuantity" id="oldQuantity"
									readonly="readonly" />
							</div>
						</div>
						<div class="row mb-4">
							<div class="col-6">
								...
								<input type="text" class="form-control ${quantityValid }"
									name="newQuantity" id="newQuantity"
									placeholder="${invoice.invoiceType.id == 1 ? 'Nhập số lượng nhập' : 'Nhập số lượng xuất'}" />
								<p class="invalid-feedback">${errors.get(product.id)}</p>
							</div>

							<div class="col-6">
								<label for="price">Đơn giá</label> <input type="text"
									class="form-control ${priceValid }" id="price" name="price"
						...
						<div class="row mb-4">
							...
							<div class="col-12">
								<label for="price">Thành tiền</label> <input type="text"
									class="form-control" id="total" readonly="readonly" />
							</div>
						</div>
				</div>
				<div class="modal-footer">
					<button type="submit" class="btn btn-danger" id="btnConfirm">Cập
						nhật</button>
